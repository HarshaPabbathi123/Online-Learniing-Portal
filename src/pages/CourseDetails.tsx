...
import { Clock, Users, BookOpen, Play, Star, CheckCircle } from 'lucide-react';
...
const CourseDetails: React.FC = () => {
    ...
    const fetchCourse = async () => {
      try {
        ...
        if (user?.role === 'student') {
          const enrollmentResponse = await axios.get('http://localhost:5001/api/enrollments/my-enrollments');
          const enrollments = enrollmentResponse.data;
          setIsEnrolled(enrollments.some((enrollment: any) => enrollment.course._id === id));
        }
      } catch (error) {
  ...
  const handleEnroll = async () => {
    ...
    setEnrolling(true);
    try {
      await axios.post(`http://localhost:5001/api/enrollments/enroll/${id}`);
      setIsEnrolled(true);
