...
import { ChevronLeft, ChevronRight, Check, Play, Book } from 'lucide-react';
    ...
    const fetchData = async () => {
      try {
        const [courseResponse, enrollmentResponse] = await Promise.all([
          ...
          axios.get('http://localhost:5001/api/enrollments/my-enrollments')
        ]);
  ...
  const handleModuleComplete = async (moduleId: string, completed: boolean) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5001/api/enrollments/progress/${enrollment._id}/${moduleId}`,
        { completed }
      );
